...
def getPullRequests(module, server, project, repository, username, password, title):
...
def main():
    ...
    if "create" in actions:
        ...
        try:
            ...
            if response.status_code == 409 and ignore_existing_on_create is True:
                module.warn(
                    'A pull-request already exists, ignoring error as requested')
            else:
                if response.ok:
                    result['changed'] = True
                else:
                    ...
                    module.fail_json(
                        msg=f"Error creating new pull request: {error_data['errors'][0]['message']}")
    ...
    if "approve" in actions:
        try:
            prs = getPullRequests(
                module, server, project, repository, username, password, title)
            mypr = {}
    ...
    if "merge" in actions:
        try:
            ...
            if mypr == {}:
                module.fail_json(
                    msg=f"Unable to find a PR that matches requested parameters")
